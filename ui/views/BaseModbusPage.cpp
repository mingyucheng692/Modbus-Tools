...
void BaseModbusPage::setupCommonConnections() {
    ...
    if (controlWidget_) {
        ...
        connect(controlWidget_, &widgets::ControlWidget::pollRequested,
            this, [this, ensureConnected](uint8_t fc, int addr, int qty) {
        ...
        connect(sessionPresenter_,
                ...
                this, [this](int requestId, const ::modbus::session::ModbusResponse& response) {
                    ...
                    if (response.isSuccess && linked_) {
                        modbus::core::parser::ProtocolType protocolType =
                            (sessionMode_ == ui::application::modbus::SessionMode::Rtu)
    ...
    if (functionWidget_) {
        ...
        connect(functionWidget_, &widgets::FunctionWidget::readRequested,
            this, [this, ensureConnected](uint8_t fc, int addr, int qty, int slaveId) {
                if (!ensureConnected()) return;
        ...
        connect(functionWidget_, &widgets::FunctionWidget::writeRequested,
            this, [this, ensureConnected](uint8_t fc, int addr, const QString& dataStr, const QString& fmt, int slaveId) {
                if (!ensureConnected()) return;
        ...
        connect(functionWidget_, &widgets::FunctionWidget::rawSendRequested,
            this, [this, ensureConnected](const QByteArray& data) {
                if (!ensureConnected()) return;
