/**
 * @file CollapsibleSection.h
 * @brief Header file for CollapsibleSection.
 * 
 * Copyright (c) 2025 - present mingyucheng692
 * 
 * Licensed under the MIT License. See LICENSE file in the project root for full license information.
 */

#pragma once

#include <QWidget>

class QLabel;
class QToolButton;
class QGroupBox;
class QEvent;
class QString;

namespace infra::config {
class ISettingsService;
}

namespace ui::widgets {

class CollapsibleSection : public QWidget {
    Q_OBJECT

public:
    explicit CollapsibleSection(infra::config::ISettingsService* settingsService, QWidget* parent = nullptr);
    QWidget* contentWidget() const;
    void setTitle(const QString& title);
    void setExpanded(bool expanded);
    bool isExpanded() const;
    void setSettingsKey(const QString& key);

signals:
    void expandedChanged(bool expanded);

private:
    void updateToggleUi();
    void updateSectionGeometry();
    void loadSettings();
    void saveSettings() const;
    void changeEvent(QEvent* event) override;

    QWidget* headerWidget_ = nullptr;
    QLabel* titleLabel_ = nullptr;
    QToolButton* toggleButton_ = nullptr;
    QGroupBox* bodyGroup_ = nullptr;
    QWidget* contentContainer_ = nullptr;
    QString settingsKey_;
    infra::config::ISettingsService* settingsService_ = nullptr;
    bool expanded_ = true;
};

} // namespace ui::widgets
