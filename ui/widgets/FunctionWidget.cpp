/**
 * @file FunctionWidget.cpp
 * @brief Implementation of FunctionWidget.
 * 
...
FunctionWidget::FunctionWidget(infra::config::ISettingsService* settingsService, QWidget *parent)
    : QWidget(parent),
...
int FunctionWidget::getSlaveId() const {
    ...
    bool ok;
    int val = ui::common::data_helper::parseSmartInt(slaveIdEdit_->text(), &ok);
    return ok ? val : 1;
}
...
void FunctionWidget::onAppendCrcClicked() {
    ...
    QByteArray data = ui::common::data_helper::parseHex(input);
