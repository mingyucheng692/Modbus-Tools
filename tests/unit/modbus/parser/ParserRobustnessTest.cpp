...
TEST(ParserRobustnessTest, TruncatedFrames) {
    ...
    ParseResult result = parse(shortFrame, ProtocolType::Rtu);
    ...
    ParseResult resultTcp = parse(tcpHeaderOnly, ProtocolType::Tcp);
...
TEST(ParserRobustnessTest, InvalidMBAPLength) {
    ...
    ParseResult result = parse(badLenFrame, ProtocolType::Tcp);
    EXPECT_FALSE(result.isValid);
}
...
TEST(ParserRobustnessTest, ProtocolViolation) {
    ...
    ParseResult result = parse(badPidFrame, ProtocolType::Tcp);
    EXPECT_FALSE(result.isValid);
    
    // RTU frame but CRC is definitely wrong
    QByteArray badCrcFrame = QByteArray::fromHex("010304007B01C8FFFF");
    ParseResult resultRtu = parse(badCrcFrame, ProtocolType::Rtu);
...
TEST(ParserRobustnessTest, FuzzLikeInput) {
    ...
    ParseResult result = parse(garbage, ProtocolType::Unknown);
    EXPECT_FALSE(result.isValid);
}
...
TEST(ParserRobustnessTest, ExceptionResponseHandling) {
    ...
    ParseResult result = parse(excFrame, ProtocolType::Rtu);
