...
>
> **轮询日志策略**：单次轮询失败记录为 `Warning`；连续失败会按轮询间隔对应的自适应阈值升级为 `Error`；恢复成功后会补充一条恢复提示。
...
- **TCP/UDP 调试助手**：支持 TCP Client、TCP Server 以及 UDP 模式，适配各类通用网络协议调试与验证。
- **串口调试助手**：支持自定义波特率、数据位、校验位，集成文件传输以及控制线引脚（DTR/RTS）控制等实用功能。
...
### 自动化与状态
- **测试通过率**：全量 42 个自动化测试用例已 100% 通过验证。
- **CI 集成**：GitHub Actions 流水线集成了 MSVC AddressSanitizer (ASan)，用于内存损坏与泄漏的自动化监测。
- **回归验证**：每次 Release 发布均执行全量回归测试，验证协议一致性与会话状态稳健性。
